...
describe('fields', () => {
  describe('getCompatibleFields', () => {
    it('returns map of compatible fields and filters content types', () => {
      const { compatibleFields, filteredContentTypes } = getCompatibleFields(contentTypes);

      expect(compatibleFields).toEqual({
      ...
      expect(filteredContentTypes).toEqual([contentTypes[0], contentTypes[2]]);
    });
  });
